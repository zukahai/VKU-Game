...
public class HighScore extends JFrame implements ActionListener{
	...
	private JButton[]lv_bt = new JButton[3];
	DefaultTableModel model;
	JTable tb;
	JScrollPane jsc;
	public HighScore (int lv) {
		super("Game Dò Mìn - VKU");
		level = lv;
		cn = init();
	}
	Container init() {
		...
		lv_bt[0].setText("Dễ");
		lv_bt[1].setText("Trung bình");
		lv_bt[2].setText("Khó");
		for (int i = 0; i < lv_bt.length; i++) {
			lv_bt[i].setActionCommand(String.valueOf(i + 1));
		}
...
//		setDefaultCloseOperation(EXIT_ON_CLOSE);
	...
	public Vector writeScore() {
		...
		try {
			URL url = new URL("https://haizukon.000webhostapp.com/HighScore/GameDoMin/level_" + level + "/vku.txt");
...
//			System.out.println(str[0]);
	...
	public void actionPerformed(ActionEvent e) {
		// TODO Auto-generated method stub
		new HighScore(Integer.parseInt(e.getActionCommand()));
		this.dispose();
	}
}
